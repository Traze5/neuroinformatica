...
def detect_bursts(t, x, thr_hi=None, thr_lo=None, min_sep=0.05):
    ...
    if x.size < 4:
        return Bursts(np.array([]), np.array([]))
    if thr_hi is None:
        thr_hi = np.percentile(x, 75)
    if thr_lo is None:
        thr_lo = np.percentile(x, 55)
    above = x > thr_hi
    below = x < thr_lo
    ...
    on, off = [], []; j = 0
    for i in onset_idx:
        while j < len(offset_idx) and offset_idx[j] <= i:
            j += 1
...
def integrate(fun, t_max, dt, x0_state, args=(), method="euler", variant="HR"):
    ...
    if method == "euler" and NUMBA:
        if variant == "HR modificado":
            return euler_hrmod_numba(*args, dt, n, *x0_state)
        else:
            return euler_hr_numba(*args, dt, n, *x0_state)
